 ...
 * @return string
 */
function spellolite_get_main_menu() {
	$args = apply_filters( 'spellolite_main_menu_args', array(
		'theme_location'   => 'main',
		'container'        => '',
		'menu_id'          => 'main-menu',
		'echo'             => false,
		'fallback_cb'      => 'spellolite_set_nav_menu',
		'fallback_message' => esc_html__( 'Set main menu', 'spellolite' ),
	) );
 ...
 * @return void
 */
function spellolite_main_menu() {

	$main_menu = spellolite_get_main_menu();
 ...
 * @return void
 */
function spellolite_vertical_main_menu( $slide = 'left', $args = array() ) {

	$default_args = apply_filters( 'spellolite_vertical_menu_args', array(
		...
		'button-back-inner-html'  => '<i class="linearicon linearicon-chevron-left"></i><span class="navigation-button__text">' . esc_html__( 'Back', 'spellolite' ) . '</span>',
		'button-close-inner-html' => '<i class="linearicon linearicon-cross"></i>',
	) );

	$args        = wp_parse_args( $args, $default_args );
	$menu        = spellolite_get_main_menu();
 ...
 * @return void
 */
function spellolite_footer_menu() {
	if ( ! get_theme_mod( 'footer_menu_visibility', spellolite_theme()->customizer->get_default( 'footer_menu_visibility' ) ) ) {
		return;
	}

	$args = apply_filters( 'spellolite_footer_menu_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'spellolite_set_nav_menu',
		'fallback_message' => esc_html__( 'Set footer menu', 'spellolite' ),
	) );
 ...
 * @return void
 */
function spellolite_top_menu() {

	if ( ! get_theme_mod( 'top_menu_visibility', spellolite_theme()->customizer->get_default( 'top_menu_visibility' ) ) ) {
		return;
	}
	...
	wp_nav_menu( apply_filters( 'spellolite_top_menu_args', array(
		'theme_location'  => 'top',
		'container'       => 'div',
 ...
 * @return string
 */
function spellolite_get_social_list( $context, $type = 'icon' ) {
	static $instance = 0;
	$instance++;
	...
	$args = apply_filters( 'spellolite_social_list_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'spellolite_set_nav_menu',
		'fallback_message' => esc_html__( 'Set social menu', 'spellolite' ),
	), $context, $type );
 ...
 * @return null|string
 */
function spellolite_set_nav_menu( $args ) {
 ...
 * @return void
 */
function spellolite_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'spellolite_menu_toggle_html',
 ...
 * @return void
 */
function spellolite_vertical_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'spellolite_vertical_menu_toggle_html',
