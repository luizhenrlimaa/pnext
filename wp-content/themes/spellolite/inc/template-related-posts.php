 ...
 * @return array
 */
function spellolite_related_posts() {
	...
	$visible = get_theme_mod( 'related_posts_visible', spellolite_theme()->customizer->get_default( 'related_posts_visible' ) );
	...
	$post_terms  = '';
	$post_number = get_theme_mod( 'related_posts_count', spellolite_theme()->customizer->get_default( 'related_posts_count' ) );
	...
	$holder_view_dir = locate_template( apply_filters( 'spellolite_related_posts_template_part_slug', 'template-parts/content-related-posts.php' ), false, false );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = get_theme_mod( $setting_value, spellolite_theme()->customizer->get_default( $setting_value ) );
	}

	$settings['title_visible']   = $settings['title_length'] ? get_theme_mod( 'related_posts_title', spellolite_theme()->customizer->get_default( 'related_posts_title' ) ) : false;
	...
	if ( $holder_view_dir ) {
		...
		$block_title_format = apply_filters( 'spellolite_rp_block_title_format', '<h5 class="entry-title">%s</h5>' );
		...
		foreach ( $posts as $post ) {
			...
			$image = $utility->media->get_image( apply_filters( 'spellolite_rp_image_settings', array(
			...
			$title = $utility->attributes->get_title( apply_filters( 'spellolite_rp_title_settings', array(
			...
			$excerpt = $utility->attributes->get_content( apply_filters( 'spellolite_rp_excerpt_settings', array(
				'visible'      => $settings['content_visible'],
				'length'       => $settings['content_length'],
				'content_type' => $settings['content_type'],
			) ) );

			$category = $utility->meta_data->get_terms( apply_filters( 'spellolite_rp_category_settings', array(
				'delimiter' => ', ',
				'type'      => 'category',
			...
			$tag = $utility->meta_data->get_terms( apply_filters( 'spellolite_rp_tag_settings', array(
				'delimiter' => ', ',
				'type'      => 'post_tag',
			...
			$author = $utility->meta_data->get_author( apply_filters( 'spellolite_rp_author_settings', array(
			...
			$date = $utility->meta_data->get_date( apply_filters( 'spellolite_rp_date_settings', array(
				'visible' => $settings['date_visible'],
				'html'    => '<span class="post__date">%1$s<a href="%2$s" %3$s %4$s><time datetime="%5$s">%6$s%7$s</time></a></span>',
				'class'   => 'post__date-link',
			) ) );

			$comment_count = $utility->meta_data->get_comment_count( apply_filters( 'spellolite_rp_comment_count_settings', array(
