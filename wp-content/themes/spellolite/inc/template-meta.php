 ...
 * @return array
 */
function spellolite_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! spellolite_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'spellolite_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'spellolite_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'spellolite' ) . $network['name'],
 ...
 * @return string
 */
function spellolite_get_single_share_buttons() {
	ob_start();
