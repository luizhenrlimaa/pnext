 ...
 * @package Spellolite
 */
if ( ! class_exists( 'Spellolite_Carousel_Widget' ) ) {

	/**
	 * Class Spellolite_Carousel_Widget.
	 */
	class Spellolite_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'spellolite' );
			$this->widget_id          = 'spellolite_widget_carousel';
			$this->widget_name        = esc_html__( 'Carousel', 'spellolite' );
			$this->utility            = spellolite_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Title', 'spellolite' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category',
					'options' => array(
						'category' => array(
							'label' => esc_html__( 'Category', 'spellolite' ),
							'slave' => 'terms_type_category',
						),
						'post_tag' => array(
							'label' => esc_html__( 'Tag', 'spellolite' ),
							'slave' => 'terms_type_post_tag',
						),
					),
					'label'   => esc_html__( 'Choose taxonomy type', 'spellolite' ),
				),
				'categories' => array(
					'type'             => 'select',
					'size'             => 1,
					...
					'options'          => false,
					'label'            => esc_html__( 'Select category', 'spellolite' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select category', 'spellolite' ),
				...
				'tags' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select tags', 'spellolite' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select tags', 'spellolite' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 20,
					'min_value' => 1,
					'label'     => esc_html__( 'Posts count', 'spellolite' ),
				),
				'post_title' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display title', 'spellolite' ),
				),
				'content' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display content', 'spellolite' ),
				),
				'more_button' => array(
					'type'   => 'switcher',
					'value'  => 'false',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display more button', 'spellolite' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'spellolite' ),
						'false_toggle' => esc_html__( 'Off', 'spellolite' ),
				...
				'more_button_text' => array(
					'type'   => 'text',
					'value'  => esc_html__( 'Read more', 'spellolite' ),
					'label'  => esc_html__( 'More button text', 'spellolite' ),
				...
				'trim_title' => array(
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Title chars trimmed count', 'spellolite' ),
				),
				'trim_words' => array(
					'type'       => 'slider',
					'value'      => 15,
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Content words trimmed count', 'spellolite' ),
				),
				'slides_per_view' => array(
					...
					'min_value' => 1,
					'value'     => 5,
					'label'     => esc_html__( 'Number of slides per view', 'spellolite' ),
				),
				'slides_per_group' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Number slides per group', 'spellolite' ),
				),
				'slides_per_column' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Multi Row Slides Layout', 'spellolite' ),
				),
				'space_between_slides' => array(
					...
					'min_value' => 0,
					'value'     => 30,
					'label'     => esc_html__( 'Width of the space between slides(px)', 'spellolite' ),
				),
				'duration_speed' => array(
					...
					'min_value' => 100,
					'value'     => 500,
					'label'     => esc_html__( 'Duration of transition between slides (ms)', 'spellolite' ),
				),
				'navigation' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider navigation', 'spellolite' ),
				),
				'pagination' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider pagination', 'spellolite' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'spellolite_carousel_widget_view_dir', 'inc/widgets/carousel/views/carousel-view.php' ), false, false );
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', spellolite_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="spellolite-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'spellolite' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'spellolite_carousel_default_query_args', array(
				'post_type'      => 'post',
				'orderby'        => 'date',
				'order'          => 'DESC',
				'posts_per_page' => - 1,
		...
		public function get_carousel_loop( $posts_query, $template ) {
			...
			if ( ! $template  ) {
				return '<h5>' . esc_html__( 'View file not found', 'spellolite' ) . '</h5>';
			}
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$image = $this->utility->media->get_image( array(
					'size'              => 'post-thumbnail',
					'mobile_size'       => 'spellolite-thumb-m',
	...
	add_action( 'widgets_init', 'spellolite_register_carousel_widget' );
	/**
	 * Register carousel widget.
	 */
	function spellolite_register_carousel_widget() {
		register_widget( 'Spellolite_Carousel_Widget' );
	}
}
