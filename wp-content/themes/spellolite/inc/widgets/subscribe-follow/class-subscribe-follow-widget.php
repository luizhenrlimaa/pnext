...
class Spellolite_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'spellolite' );
		$this->widget_id          = 'spellolite_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'spellolite' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'spellolite' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'spellolite' ),
				'label' => esc_html__( 'Subscribe Title', 'spellolite' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'spellolite' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'spellolite' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'spellolite' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'spellolite' ),
				'label' => esc_html__( 'Subscribe submit label', 'spellolite' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'spellolite' ),
				'label' => esc_html__( 'Subscribe success', 'spellolite' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'spellolite' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'spellolite' ),
				'label' => esc_html__( 'Follow Title', 'spellolite' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'spellolite' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'spellolite' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'spellolite' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'spellolite' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'spellolite' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'spellolite' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'spellolite' ),
					'top-center'    => esc_html__( 'Top Center', 'spellolite' ),
					'top-right'     => esc_html__( 'Top Right', 'spellolite' ),
					'center-left'   => esc_html__( 'Middle Left', 'spellolite' ),
					'center'        => esc_html__( 'Middle Center', 'spellolite' ),
					'center-right'  => esc_html__( 'Middle Right', 'spellolite' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'spellolite' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'spellolite' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'spellolite' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'spellolite' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'spellolite' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'spellolite' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'spellolite' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'spellolite' ),
					'contain' => esc_html__( 'Contain', 'spellolite' ),
					'auto'    => esc_html__( 'Auto', 'spellolite' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'spellolite' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_spellolite_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_spellolite_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'spellolite' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'spellolite' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'spellolite' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'spellolite' ),
		);

		$this->nonce = wp_create_nonce( 'spellolite-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = locate_template( apply_filters( 'spellolite_subscribe_widget_follow_template_file', 'inc/widgets/subscribe-follow/view/follow-view.php' ), false, false );
		$subscribe_template         = locate_template( apply_filters( 'spellolite_subscribe_widget_subscribe_template_file', 'inc/widgets/subscribe-follow/view/subcribe-view.php' ), false, false );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'spellolite' );
		}
	...
	public function get_social_nav() {
		return spellolite_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = spellolite_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'spellolite-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'spellolite_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function spellolite_register_subscribe_follow_widgets() {
	register_widget( 'Spellolite_Subscribe_Follow_Widget' );
}
